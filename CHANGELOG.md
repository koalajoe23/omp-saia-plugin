...
### Fixed
- `omp plugin install` no longer hangs: the reconciler's background schedule was armed from the extension factory body using raw, ref'd `setInterval`/`setTimeout`. The installer loads and *runs* that factory to validate the extension (`PluginManager.install` -> `loadExtensions`), in a CLI process that exits by draining the event loop, so the 6 h interval held it open forever. The schedule is now armed on `session_start` through OMP's managed timers (`ctx.setInterval`/`ctx.setTimeout` — unref'd, throw-contained, cleared on teardown) and disarmed on `session_shutdown`; the fallback timer host unrefs as well, so no code path can pin a short-lived process again
