...
- **Shortening omp's 24h discovery cache** — investigated: the extension API exposes no model-cache invalidation or TTL knob (`modelRegistry`/`models` are read-only; no refresh API). The reconciler keeps its own store fresh so every omp re-discovery (24h tick, `omp models`, `omp models refresh`, fresh start) lands on fresh data. Whether `packages/catalog/src/model-cache.ts` exposes a per-provider TTL override is an investigation item for the implementation plan; likely outcome: not possible without an omp feature.
